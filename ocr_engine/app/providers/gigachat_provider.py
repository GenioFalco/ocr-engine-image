...
КРИТИЧЕСКИ ВАЖНО — различай эти поля:
- `document_number` = НОМЕР САМОГО ДОКУМЕНТА из его заголовка (например, "Счёт-фактура № 14219/065" → "14219/065"). НЕ номер договора!
- `document_date` = ДАТА САМОГО ДОКУМЕНТА из заголовка (например, "от 17.03.2023"). НЕ дата договора!
- `basis_document.number` = номер договора/основания из строки "К договору №..." или "Основание: договор №..."
- `basis_document.date` = дата договора/основания из той же строки про договор
- `seller` = продавец/исполнитель/поставщик (кто выставил документ)
- `buyer` = покупатель/заказчик (кому выставлен документ)
            """
