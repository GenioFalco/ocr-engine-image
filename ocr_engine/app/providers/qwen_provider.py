...
4. ПРАВИЛА ДЛЯ ИНН / КПП (КРИТИЧЕСКИ ВАЖНО — НЕ ПУТАЙ):
   ...
   - КАК ЧИТАТЬ СТРОКУ "ИНН/КПП": в российских документах часто пишут "ИНН/КПП продавца: 3305051742/330501001"
     * Число ДО знака "/" — это ИНН (10 или 12 цифр) → пиши в поле `inn`
     * Число ПОСЛЕ знака "/" — это КПП (9 цифр) → пиши в поле `kpp`
     * НЕ сливай ИНН и КПП в одно поле! "3305051742/330501001" → inn="3305051742", kpp="330501001"
     * НЕ путай с индексом — индекс стоит в адресе (6 цифр перед городом), ИНН стоит в строке "ИНН" или "ИНН/КПП"
